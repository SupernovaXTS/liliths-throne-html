...
(function () {
  ...
  function pill(active, act, label, colour) {
    ...
    if (active) {
      return (
        '<div class="cosmetics-button active"><span style="color:' +
        c +
        ';">' +
        label +
        "</span></div>"
  ...
  function stepList(list, current, dir) {
    var i = 0;
    for (; i < list.length; i++)
      if (list[i] === current || list[i].id === current.id) break;
  ...
  function coreHtml() {
    ...
    return (
      ...
      pills(
        "Body Size",
        "How much fat your body carries.",
        LT.BODY_SIZE_LIST,
        p.bodySize.id,
        "SIZE_",
      ) +
  ...
  function faceHtml() {
    ...
    return (
      ...
      toggle(
        "Puffy Lips",
        "Whether your lips are extra puffy.",
        "LIPPUFF",
        p.lipsPuffy,
        "Puffy",
        "Natural",
      ...
      pills(
        "Iris Colour",
        "The colour of your eye's irises.",
        LT.EYE,
        p.eye.id,
        "EYE_",
      )
  ...
  function hairHtml() {
    ...
    return (
      ...
      pills(
        "Hair Style",
        "Choose your hair style. Certain hair styles are unavailable at shorter hair lengths.",
        styles,
        p.hairStyle.id,
        "HSTYLE_",
      ) +
  ...
  function breastsHtml() {
    ...
    return (
      ...
      toggle(
        "Puffy Nipples",
        "Whether your nipples are puffy.",
        "NIPPUFF",
        p.nipplesPuffy,
        "Puffy",
        "Natural",
      ...
      pills(
        "Lactation",
        "How much milk your breasts produce.",
        LT.LACTATION,
        (p.body && p.body.breast && (p.body.breast.lactation || "ZERO_NONE")) ||
          "ZERO_NONE",
        "LACT_",
      )
    );
  ...
  function assHtml() {
    ...
    return (
      ...
      toggle(
        "Bleached Anus",
        "Whether you've bleached around your anus.",
        "BLEACH",
        p.anusBleached,
        "Bleached",
        "Natural",
  ...
  function genitalsHtml() {
    ...
    if (p.hasPenis()) {
      return (
        ...
        pills(
          "Cum production",
          "How much cum you produce.",
          LT.CUM_PRODUCTION,
          (p.body &&
            p.body.penis &&
            p.body.penis.testicle &&
            (p.body.penis.testicle.cumProduction || "THREE_AVERAGE")) ||
            "THREE_AVERAGE",
          "CUM_",
        )
      );
    ...
    return (
      ...
      pills(
        "Capacity",
        "How accommodating your vagina is.",
        bodyEnums.SIZE5,
        p.vaginaCapacity.id,
        "CAP_",
      ) +
  ...
  function piercingsHtml() {
    ...
    (LT.PIERCING_TYPES || []).forEach(function (slot) {
      ...
      if (!ok) {
        html +=
          '<div class="cosmetics-button disabled"><span style="opacity:0.45;">Unpierced</span></div>';
        html +=
          '<div class="cosmetics-button disabled"><span style="opacity:0.45;">Pierced</span></div>';
      } else {
  ...
  function tattoosHtml() {
    ...
    (LT.TATTOO_SLOTS || []).forEach(function (slot) {
      ...
      if (blocked) {
        ...
        html +=
          "<p>" +
          (tat.type && tat.type !== "NONE"
            ? tat.type.replace(/_/g, " ")
            : "writing") +
  ...
  function tattooAddHtml() {
    ...
    var draft = LT.game.flags.creationTattooDraft || {
      type: "hearts",
      colour: "BLACK",
      writing: "",
    };
    var slot = bodyEnums.findById(LT.TATTOO_SLOTS, slotId) || {
      id: slotId,
      name: slotId,
    };
    return (
      ...
      pills(
        "Tattoo type",
        "Mundane designs available during character creation.",
        LT.TATTOO_TYPES,
        draft.type,
        "TTYPE_",
      ) +
      pills(
        "Primary colour",
        "The main colour of this tattoo.",
        LT.MAKEUP_COLOURS.filter(function (c) {
          return c.id !== "NONE";
        }),
        draft.colour,
        "TCOL_",
  ...
  function bodyHairHtml() {
    ...
    html += pills(
      "Body hair colour",
      "This is the hair that covers all areas other than the head.",
      LT.HAIR_COLOUR,
      (b.coverings && b.coverings.BODY_HAIR && b.coverings.BODY_HAIR.primary) ||
        "BROWN",
      "BHAIRCOL_",
    );
    if (feminine) {
      ...
      html += pills(
        "Facial hair",
        "The body hair found on your face.",
        LT.BODY_HAIR,
        b.facialHair,
        "FACIAL_",
      );
    ...
    html += pills(
      "Pubic hair",
      "The body hair found in the genital area; located on and around your sex organs and crotch.",
      LT.BODY_HAIR,
      b.pubicHair,
      "PUBIC_",
    );
    html += pills(
      "Underarm hair",
      "The body hair found in your armpits.",
      LT.BODY_HAIR,
      b.underarmHair,
      "UNDERARM_",
    );
    html += pills(
      "Ass hair",
      "The body hair found around your asshole.",
      LT.BODY_HAIR,
      b.assHair,
      "ASSHAIR_",
    );
  ...
  function backToHub() {
    return [
      new LT.Response(
        "Back",
        "Confirm your choices and return to the appearance menu.",
        "creation.advanced",
      ),
    ];
  }
  ...
  function handleAct(act) {
    ...
    if (act === "HEIGHT_DEC") p.heightCm = Math.max(140, p.heightCm - 1);
      ...
      p.skin = bodyEnums.findById(LT.SKIN, act.slice(5));
      ...
      p.eye = bodyEnums.findById(LT.EYE, act.slice(4));
      ...
      if (p.hairStyle.minLength > li)
        p.hairStyle = bodyEnums.findById(
          LT.HAIR_STYLE,
          li === 0 ? "NONE" : "MESSY",
        );
    } else if (act.indexOf("HSTYLE_") === 0)
      p.hairStyle = bodyEnums.findById(LT.HAIR_STYLE, act.slice(7));
    else if (act.indexOf("HCOLOUR_") === 0)
      p.hair = bodyEnums.findById(LT.HAIR_COLOUR, act.slice(8));
    else if (act.indexOf("CUP_") === 0)
      p.breastSize = LT.CUP[act.slice(4)] || p.breastSize;
    else if (act.indexOf("BSHAPE_") === 0)
      p.breastShape = bodyEnums.findById(LT.BREAST_SHAPE, act.slice(7));
    else if (act.indexOf("NIP_") === 0 && act.indexOf("NIPPUFF") !== 0)
      p.nippleSize = bodyEnums.findById(bodyEnums.SIZE5, act.slice(4));
    else if (act.indexOf("ARE_") === 0)
      p.areolaeSize = bodyEnums.findById(bodyEnums.SIZE5, act.slice(4));
    else if (act === "NIPPUFF_ON") p.nipplesPuffy = true;
    else if (act === "NIPPUFF_OFF") p.nipplesPuffy = false;
    else if (act.indexOf("ASS_") === 0)
      p.assSize = bodyEnums.findById(bodyEnums.SIZE5, act.slice(4));
    else if (act.indexOf("HIP_") === 0)
      p.hipSize = bodyEnums.findById(bodyEnums.SIZE5, act.slice(4));
    else if (act === "BLEACH_ON") p.anusBleached = true;
    else if (act === "BLEACH_OFF") p.anusBleached = false;
    else if (act.indexOf("BALLS_") === 0)
      p.testicleSize = bodyEnums.findById(bodyEnums.SIZE5, act.slice(6));
    else if (act.indexOf("CAP_") === 0)
      p.vaginaCapacity = bodyEnums.findById(bodyEnums.SIZE5, act.slice(4));
    else if (act.indexOf("LABIA_") === 0)
      p.labiaSize = bodyEnums.findById(bodyEnums.SIZE5, act.slice(6));
    else if (act.indexOf("CLIT_") === 0)
      p.clitorisSize = bodyEnums.findById(bodyEnums.SIZE5, act.slice(5));
      ...
      LT.game.flags.creationTattooDraft = {
        type: "hearts",
        colour: "BLACK",
        writing: "",
      };
      ...
      p.body.coverings.BODY_HAIR = {
        type: "HUMAN",
        primary: act.slice(9),
        secondary: act.slice(9),
        pattern: "NONE",
        modifier: "SMOOTH",
      };
    } else return;
  ...
  LT.defineNode({
    ...
    getResponses: function () {
      ...
      return [
        ...
        new LT.Response(
          "Core",
          "Enter the customisation menu for all of your body's core aspects.",
          "creation.core",
        ),
        new LT.Response(
          "Face",
          "Enter the customisation menu for aspects related to your face.",
          "creation.face",
        ),
        new LT.Response(
          "Hair",
          "Enter the customisation menu for your hair.",
          "creation.hair",
        ),
        new LT.Response(
          "Breasts",
          "Enter the customisation menu for your breasts.",
          "creation.breasts",
        ),
        new LT.Response(
          "Ass & Hips",
          "Enter the customisation menu for aspects related to your ass, hips, and anus.",
          "creation.ass",
        ),
        new LT.Response(
        ...
        new LT.Response(
          "Makeup",
          "Enter the customisation menu for makeup.",
          "creation.makeup",
        ),
        new LT.Response(
          "Piercings",
          "Enter the customisation menu for body piercings.",
          "creation.piercings",
        ),
        new LT.Response(
          "Tattoos",
          "Enter the customisation menu for tattoos.",
          "creation.tattoos",
        ),
        new LT.Response(
          "Extra hair",
          "Enter the customisation menu for facial, pubic, and body hair.",
          "creation.body-hair",
        ),
      ];
    },
  ...
  LT.defineNode({
    ...
    title: function () {
      return LT.game.player && LT.game.player.hasPenis()
        ? "Penis Appearance"
        : "Vagina Appearance";
    },
    chrome: { left: true, right: false },
  ...
  LT.defineNode({
    ...
    ui: "creation-tattoos",
    title: function () {
      var slot = bodyEnums.findById(
        LT.TATTOO_SLOTS,
        LT.game.flags && LT.game.flags.creationTattooSlot,
      );
    ...
    getResponses: function () {
      var draft = (LT.game.flags && LT.game.flags.creationTattooDraft) || {
        type: "hearts",
        colour: "BLACK",
        writing: "",
      };
      var apply = new LT.Response(
        "Apply",
        "Add this tattoo.",
        "creation.tattoos",
        function () {
          ...
          ensureAppearanceExtras(p);
          var slotId = LT.game.flags.creationTattooSlot;
          var next = LT.game.flags.creationTattooDraft || {
            type: "hearts",
            colour: "BLACK",
            writing: "",
          };
      ...
      if (draft.type === "NONE" && !draft.writing) {
        apply.disable(
          "You need to select a tattoo type or add some writing in order to make a tattoo!",
        );
      }
      return [
        apply,
        new LT.Response(
          "Back",
          "Decide not to get this tattoo and return to the main selection screen.",
          "creation.tattoos",
        ),
      ];
    },
